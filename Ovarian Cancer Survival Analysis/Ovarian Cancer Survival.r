...
plot(kaplan1, xlab="Time (in days)", ylab="Survival", main="Kaplan-Meier Analysis")

# Kaplan-Meier non-parametric by group (treatment type)
kaplan2 <- survfit(Surv(time, status) ~ treatment)
summary(kaplan2)

plot(kaplan2, xlab="Time (in days)", ylab="Survival", main="Kaplan-Meier Analysis Depending on Treatment",
     col=c("red", "blue"))
